...
export default function Search({ courses }) {
  ...
  useEffect(() => {
    $(function () {
      ...
      else $("." + styles.search_cat_opt + "[data-cat='diff']").slideDown(300);
      if (!langMenu)
        $("." + styles.search_cat_opt + "[data-cat='lang']").slideUp(300);
      else $("." + styles.search_cat_opt + "[data-cat='lang']").slideDown(300);
      $("." + styles.search_cat_opt_elem_icon).hide();
      if (diff == "all")
        $("." + styles.search_cat_opt + "[data-cat='diff']")
          .children("." + styles.search_cat_opt_all)
          .children("." + styles.search_cat_opt_elem_icon)
          .show();
      else
        diff.map((tag) => {
          $("." + styles.search_cat_opt + "[data-cat='diff']")
            .children("." + styles.search_cat_opt_elem + `[data-tag='${tag}']`)
            .children("." + styles.search_cat_opt_elem_icon)
            .show();
        });
      if (lang == "all")
        $("." + styles.search_cat_opt + "[data-cat='lang']")
          .children("." + styles.search_cat_opt_all)
          .children("." + styles.search_cat_opt_elem_icon)
          .show();
      else
      ...
      $("." + styles.search_cat_text).on("click", function () {
        if ($(this).siblings().data("cat") == "diff") setDiffMenu(!diffMenu);
        else setLangMenu(!langMenu);
      });
      $("." + styles.search_cat_opt_elem).on("click", function () {
        if ($(this).parent().data("cat") == "diff") {
          ...
          if (lang.includes($(this).data("tag")))
            ...
            setLang(
              lang.filter((str) => str != "all").concat([$(this).data("tag")])
            );
        }
      });
  ...
  const searchCourses = useCallback(() => {
    let newCourses = courses.filter(
      (course) =>
        (diff.includes(course.difficulty) || diff == "all") &&
        (lang.includes(course.language) || lang == "all")
    );
    const options = {
      includeScore: false,
      keys: ["name", "tags"],
    ...
    if (newCourses.length > 0)
      return newCourses.map((obj) => {
        return <Card key={obj.name} data={obj} />;
      });
    else
      return (
    ...
    <>
      <div className={styles.search}>
        ...
        <div className={styles.search_cat}>
          <h1 className={styles.search_cat_text}>
            Difficulty &nbsp;
            <FontAwesomeIcon icon={faChevronDown} />
          </h1>
          <div
            className={styles.search_cat_opt}
            data-cat="diff"
            style={{ display: "none" }}
          >
            <div className={styles.search_cat_opt_all} data-cat="diff">
              <h2>All</h2>
              <FontAwesomeIcon
                className={styles.search_cat_opt_elem_icon}
                icon={faCheck}
              />
            </div>
      ...
      <div className={styles.course_cont}>{searchCourses()}</div>
    </>
  );
}
